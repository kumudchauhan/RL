...
class LineItemVerifier(Verifier):
    ...
    def _match_score(self, pred: LineItem, truth: LineItem) -> float:
        ...
        qty_score = 1.0 if pred.quantity == truth.quantity else 0.0
        ...
        return name_sim * 0.4 + qty_score * 0.2 + price_score * 0.4
    ...
    def _greedy_match(
        ...
        for i, pred in enumerate(predictions):
            for j, truth in enumerate(truths):
                s = self._match_score(pred, truth)
                if s > 0.3:
                    scores.append((s, i, j))
    ...
    def verify(
        ...
        matches = self._greedy_match(pred_items, true_items)
